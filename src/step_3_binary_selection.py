...
def main(ppl_file, output_file, sql_file1, sql_file2, x=0):
    ...
    for i in tqdm(range(x, len(ppls))):
        ...
        table_info, candidate_sql = prompt_construct(
            simple_ddl, ddl_data, foreign_key, explanation, ppl, sql1, sql2
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sql_3_output", type=str, default="src/sql_log/step_3_binary.txt"
    )
    parser.add_argument("--sql_1", type=str, default="src/sql_log/preliminary_sql.txt")
    parser.add_argument(
        "--sql_2", type=str, default="src/sql_log/step_2_information_augmentation.txt"
    )
