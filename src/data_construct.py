...
import os
from configs.config import dev_json_path, type
...
def create_output_structure(ppls, table_infos):
    ...
    for i in tqdm(range(len(ppls))):
        ...
        table_str = next(
            (info["simplified_ddl"] for info in table_infos if info["db"] == db_name),
            None,
        )
...
def generate_ppl_dev_json(dev_file, out_file):
    ...
    all_table_info = [
        {"db": db_list[i], "simplified_ddl": table_str_list[i]}
        for i in range(len(db_list))
    ]
    save_json(all_table_info, f"src/information/{type}/describle.json")

    table_infos = load_json(f"src/information/{type}/describle.json")
    outputs = create_output_structure(ppls, table_infos)

    save_json(outputs, out_file)

    os.remove(f"src/information/{type}/describle.json")


generate_ppl_dev_json(dev_json_path, f"src/information/{type}/ppl_dev.json")
print("dev_json_path: ", dev_json_path)
