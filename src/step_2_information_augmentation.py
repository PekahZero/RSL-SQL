...
from configs.Instruction import (
...
def main(ppl_file, output_file, info_file, x):
    ...
    for i in tqdm(range(x, len(ppls))):
        ...
        if type == "bird":
            ...
            explanation = ""
        # table_info
        table_info = table_info_construct(
            simple_ddl, ddl_data, foreign_key, explanation
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sql_2_output",
        type=str,
        default="src/sql_log/step_2_information_augmentation.txt",
    )
    parser.add_argument(
        "--information_output", type=str, default="src/information/augmentation.json"
    )
    # 解析命令行参数
