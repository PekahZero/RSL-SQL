...
class BasicExampleSelector(object):
    def __init__(self):
        ...
        ##得到所有的问题和sql语句
        with open(f"few_shot/{type}/QA.json", "r") as jsonl_f:
...
class EuclideanDistanceQuestionMaskSelector(BasicExampleSelector):
    def __init__(self, *args, **kwargs):
        ...
        self.mask_token = (
            "<mask>"  # the "<mask>" is the mask token of all-mpnet-base-v2
        )
        self.value_token = (
            "<unk>"  # the "<unk>" is the unknown token of all-mpnet-base-v2
        )
        ...
        self.train_embeddings = self.bert_model.encode(
            train_mask_questions, show_progress_bar=False
        )
    ...
    def get_examples(self, target, num_example):
        ...
        target_embedding = self.bert_model.encode(
            target_mask_question, show_progress_bar=False
        ).reshape(1, -1)
